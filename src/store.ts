...
// Default TypeScript config for Vue 2.7
...
export function useStore(
  ...
  function init() {
    ...
    watch(
      ...
      ([filename, code]) => {
        // Skip config files (import map, tsconfig)
        if (filename === importMapFile || filename === tsconfigFile) return
    ...
    // Init tsconfig file if not exists
    if (!files.value[tsconfigFile]) {
      set(files.value, tsconfigFile, createFile(
        tsconfigFile,
        JSON.stringify(defaultTsConfig, undefined, 2)
      ))
    }
    ...
    for (const [filename, file] of Object.entries(files.value)) {
      // Skip config files (import map, tsconfig)
      if (filename === importMapFile || filename === tsconfigFile) continue
  ...
  const getTsConfig: ReplStore['getTsConfig'] = () => {
    try {
      return JSON.parse(files.value[tsconfigFile]?.code || JSON.stringify(defaultTsConfig))
    } catch (e) {
      // Append error instead of overwriting
      const errorMsg = `Syntax error in ${tsconfigFile}: ${(e as Error).message}`
      if (!errors.value.includes(errorMsg)) {
  ...
  // Placeholder for language tools reload - will be set by Monaco env
  let reloadLanguageTools: (() => void) | undefined

  const serialize = (): string => {
    const exportedFiles: Record<string, string> = {}
    for (const [filename, file] of Object.entries(files.value)) {
      const normalized = stripSrcPrefix(filename)
      // Skip tsconfig (it's a local config, not part of the shareable state)
      if (normalized === tsconfigFile) {
        continue
      }
  ...
  const setFiles = async (
    ...
    // Preserve config files (import-map and tsconfig)
    if (files.value[importMapFile]) {
      filesObj[importMapFile] = files.value[importMapFile]
    }
    if (files.value[tsconfigFile]) {
      filesObj[tsconfigFile] = files.value[tsconfigFile]
    }
    ...
    for (const file of Object.values(filesObj)) {
      // Skip config files
      if (file.filename === importMapFile || file.filename === tsconfigFile) continue
  ...
  if (!files.value[mainFile.value]) {
    mainFile.value = Object.keys(files.value).find(f => f !== importMapFile && f !== tsconfigFile) || defaultMainFile
  }
  ...
  const store: ReplStore = reactive({
    ...
    setImportMap,
    getTsConfig,
    get reloadLanguageTools() {
      return reloadLanguageTools
    },
    set reloadLanguageTools(fn: (() => void) | undefined) {
      reloadLanguageTools = fn
    },
    serialize,
    deserialize,
    getFiles,
