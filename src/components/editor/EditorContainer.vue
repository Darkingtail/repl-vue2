...
<script lang="ts">
import { type Component, type PropType, defineComponent, inject } from 'vue'
import FileSelector from './FileSelector.vue'
import { type ReplStore, importMapFile, tsconfigFile } from '../../store'
  ...
  setup(props) {
    ...
    const handleChange = debounce((code: string) => {
      if (store.activeFile) {
        ...
        // Skip compilation for config files
        const filename = store.activeFile.filename
        if (filename === importMapFile || filename === tsconfigFile) {
          return
        }
