<template>
  <div
    ref="fileSelector"
    class="file-selector"
    :class="{ 'has-config': showImportMap || showTsConfig }"
  ...
  >
    <div v-for="(file, i) in sortedFiles" :key="file" class="file-item">
      ...
      >
        <span v-if="fileIsModified(file)" class="modified-dot" />
    ...
    <div v-if="showImportMap || showTsConfig" class="config-wrapper">
      <div
        v-if="showTsConfig && files[tsconfigFile]"
        class="file"
        :class="{ active: activeFilename === tsconfigFile }"
        @click="$emit('select', tsconfigFile)"
      >
        <span class="label">TS Config</span>
      </div>
      <div
        v-if="showImportMap"
        class="file"
...
<script lang="ts">
import { type Directive, type PropType, computed, defineComponent, inject, ref } from 'vue'
import { type File, type ReplStore, tsconfigFile } from '../../store'
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // Sort files: main file first, import-map and tsconfig excluded (shown separately), others alphabetically
    const sortedFiles = computed(() => {
      const entries = Object.entries(props.files)
        .filter(([name, f]) => !f.hidden && name !== 'import-map.json' && name !== tsconfigFile)
        .map(([name]) => name)
    ...
    function doneNameFile() {
      ...
      if (!/\.(vue|jsx?|tsx?|css|s[ac]ss|less|json)$/.test(filename)) {
        alert('Only *.vue, *.js, *.ts, *.jsx, *.tsx, *.css, *.scss, *.sass, *.less, *.json files are supported.')
        return
      }
